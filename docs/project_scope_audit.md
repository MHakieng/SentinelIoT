...
| React dashboard ve interaktif topoloji | Dashboard, scan/monitor/topology/metrics/LLM panelleri iceriyor. | Uyumlu | Topoloji "runtime visualization" olarak anlatilmali, otomatik enterprise topology discovery gibi sunulmamali. |
...
Proje hedefinden uzaklasmamis. Cekirdek hedef; ag gorunurlugu, servis/CVE gorunurlugu, pasif monitoring, anomali sinyali, risk skoru, dashboard ve offline ML benchmark olarak tutarli. En buyuk kapsam riski, N-BaIoT benchmark sonuclarinin canli sistem basarisi gibi algilanmasi ve `/metrics` endpointindeki statik "real_world_metrics" alanlaridir.
...
- "Gercek dunya metrikleri" ifadesi kullanilmamali; mevcut `/metrics` icindeki `real_world_metrics` statiktir.
